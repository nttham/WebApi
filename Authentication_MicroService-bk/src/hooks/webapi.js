...
WebApi.prototype.manageWebApi = function (app) {
    ...
    app.post("/webapi/generate/*", function (req, res) {
      ...
      var path = req.path.split('/');
        ...
        var arr = path.slice(3,path.length);
        console.log("req arr "+arr)
        if(req.headers.token ){

            req.apiType = constants.generate;
    ...
    app.post("/webapi/validate", function (req, res) {

        if(req.headers.token ){

            req.apiType = constants.validate;
            var performRequest = function(err,inputRequest) {
